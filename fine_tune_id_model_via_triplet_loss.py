...
# logs based on end-face images. Read in a pre-trained model, then
# free up all model weights and fine-tune the model.
#
# Eero Holmstrom (2021)
#
...
from tensorflow.keras.layers import Input, Layer, RandomFlip, RandomRotation, RandomZoom, RandomContrast
import math
...
# of triplet distances given in the format
#
# < |xa - xp|**2 for triplet 1> < |xa - xn|**2 for triplet 1>
# < |xa - xp|**2 for triplet 2> < |xa - xn|**2 for triplet 2>
# ...
#
#
...
# three embeddings), and computes and returns the distances |xa -
# xp|**2 and |xa - xn|**2 as a tuple.
#
...
if len(sys.argv) < 10:
    ...
    print("Usage: %s [number of epochs] [learning rate] [save model every n epochs] [training data directory] [validation data directory] [triplet loss margin] [training batch size] [embedding model file] [CNN name]" % sys.argv[0])
    exit(1)
...
training_batch_size = int(sys.argv[7])
embedding_model_file_name = str(sys.argv[8])
cnn_name = str(sys.argv[9])
...
print("cnn name = %s" % cnn_name)
print("")
print("Embedding model will be read in from the file %s" % embedding_model_file_name)
print("")
...
if cnn_name == "ResNet50V2":
    ...
    print("ERROR! Unknown CNN model %s requested. Exiting." % cnn_name)
    exit(1)
...
#
# For most CNNs, preproceessing here means scaling pixel values to the
...
#
# Load the base CNN for reference
#
...
embedding_model = load_model(embedding_model_file_name)


#
# Free all CNN layers of the embedding model
#

print("")
print("Training the entire model, i.e., freeing all model weights.")
...
# model outputs the squared distance between the anchor and the
...
#
#   (1) |f(a) - f(n)|**2 < |f(a) - f(p)|**2 + dynamic_triplet_loss
#
...
#
# Before starting training, compute the validation accuracy. You can
# use this to check that the model was loaded in and set up correctly.
#

#
# First, compute the embedding for each image in the validation set
#

print("")
print("Computing embedding for each image in the validation set.")
...
print("Now computing identification accuracy over validation data.")
...
for i_epoch in range(1, epochs+1):
    ...
    #
    # Compute the embedding for each image in the training set using the
    # latest version of the embedding model.
    #
    ...
    for i_batch in range(0, math.ceil(training_data.shape[0] / training_batch_size)):
        ...
        #
        # Use gradient tape to compute gradients of the trainable
        # variables with respect to the triplet loss.
        #
